...
def load_and_preprocess_data(cv_path, job_desc_path):
    ...
    """
    try:
        print(f"Loading CV data from {cv_path} and job data from {job_desc_path}...")
        cv_data = pd.read_csv(cv_path)
        job_data = pd.read_csv(job_desc_path)
        print("CV data and job description data loaded successfully.")
...
def file_upload():
    try:
        ...
        if cv_data is None or job_data is None:
            print("Error: Unable to load or preprocess data.")
